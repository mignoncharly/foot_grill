...
function updateUnitDisplay(category, item = null) {
    ...
    if (category === 'drinks' && item) {
        const unit = translations[currentLanguage].units.drinks[item];
        unitAddon.textContent = unit;
        unitHelp.textContent = `Enter quantity in ${unit}`;
        
        // Set appropriate step value
    ...
    if (translations[currentLanguage].units[category]) {
        const unit = translations[currentLanguage].units[category];
        unitAddon.textContent = unit;
        unitHelp.textContent = `Enter quantity in ${unit}`;
...
function updateAdminList() {
    ...
    contributions.forEach((contribution) => {
        ...
        // Get the appropriate unit
        let unit;
        if (contribution.category === 'drinks') {
            unit = translations[currentLanguage].units.drinks[contribution.item];
        } else {
...
async function deleteContribution(id) {
    ...
    try {
        const response = await fetch(`/api/contributions/delete/${id}/`, {
            method: 'DELETE',
